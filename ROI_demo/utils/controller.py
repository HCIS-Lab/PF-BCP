...
class MainWindow_controller(QMainWindow):
    def __init__(self, args):
        ...
        self.model_list = ["FF", "DSA", "RRL", "BP", "BCP", "TP+BCP", "BS+BCP", "OFDE", "OADE", "PF+BCP"]
    ...
    def gen_gif(self):
        ...
        gif_save_folder = os.path.join(self.vis_result_path, "gif", model, data_type, basic)
        img_save_folder = os.path.join(self.vis_result_path, "img", model, data_type, basic, variant)
        ...
        make_video(gif_save_path, img_save_folder, variant_path, roi, behavior, risky_id, self.FPS, self.print_text)
    ...
    def gen_json(self):
        ...
        json_save_folder = os.path.join(self.vis_result_path, "json", model, data_type)
    ...
    def show_result(self):
        ...
        self.ui.f1_label.setText(f"{'OT-F1':9s} : {self.metric_result['f1-Score']*100:.1f}%")
        self.ui.recall_label.setText(f"{'OT-R':9s} : {self.metric_result['recall']*100:.1f}%")
        self.ui.precision_label.setText(f"{'OT-P':9s} : {self.metric_result['precision']*100:.1f}%")

        self.ui.pic_fa_label.setText(f"{'PIC':9s} : {self.metric_result['PIC']:.1f}")
        self.ui.wMOTA_label.setText(f"{'wMOTA':9s} : {self.metric_result['wMOTA']*100:.1f}%")
        
        self.ui.consistency_label_1.setText(f"{'OT-F1-1s':9s} : {self.metric_result['OT-F1-1s']*100:.1f}%")
        self.ui.consistency_label_2.setText(f"{'OT-F1-2s':9s} : {self.metric_result['OT-F1-2s']*100:.1f}%")
        self.ui.consistency_label_3.setText(f"{'OT-F1-3s':9s} : {self.metric_result['OT-F1-3s']*100:.1f}%")


        if self.pre_metric_result != None:

            diff_f1 = self.metric_result['f1-Score'] - self.pre_metric_result['f1-Score']
            diff_recall = self.metric_result['recall'] - self.pre_metric_result['recall']
            diff_precision = self.metric_result['precision'] - self.pre_metric_result['precision']
            set_color(self.ui.f1_diff_label, diff_f1, f"({diff_f1*100:+.1f}%)")
            set_color(self.ui.recall_diff_label, diff_recall, f"({diff_recall*100:+.1f}%)")
            set_color(self.ui.precision_diff_label, diff_precision, f"({diff_precision*100:+.1f}%)")

            diff_pic = self.metric_result['PIC'] - self.pre_metric_result['PIC']
            diff_wMOTA = self.metric_result['wMOTA'] - self.pre_metric_result['wMOTA']
            set_color(self.ui.pic_fa_diff_label, diff_pic, f"({diff_pic:+.1f})", reverse_color=True)
            set_color(self.ui.wMOTA_diff_label, diff_wMOTA, f"({diff_wMOTA*100:+.1f}%)")

            diff_consistency_1 = self.metric_result['OT-F1-1s'] - self.pre_metric_result['OT-F1-1s']
            diff_consistency_2 = self.metric_result['OT-F1-2s'] - self.pre_metric_result['OT-F1-2s']
            diff_consistency_3 = self.metric_result['OT-F1-3s'] - self.pre_metric_result['OT-F1-3s']
            set_color(self.ui.consistency_diff_label_1, diff_consistency_1, f"({diff_consistency_1*100:+.1f}%)")
            set_color(self.ui.consistency_diff_label_2, diff_consistency_2, f"({diff_consistency_2*100:+.1f}%)")
            set_color(self.ui.consistency_diff_label_3, diff_consistency_3, f"({diff_consistency_3*100:+.1f}%)")
    ...
    def show_no_result(self):
        ...
        self.ui.f1_label.setText(f"{'OT-F1':9s} : None")
        self.ui.recall_label.setText(f"{'OT-R':9s} : None")
        self.ui.precision_label.setText(f"{'OT-P':9s} : None")

        self.ui.pic_fa_label.setText(f"{'PIC':9s} : None")
        self.ui.wMOTA_label.setText(f"{'wMOTA':9s} : None")

        self.ui.consistency_label_1.setText(f"{'OT-F1-1s':9s} : None")
        self.ui.consistency_label_2.setText(f"{'OT-F1-2s':9s} : None")
        self.ui.consistency_label_3.setText(f"{'OT-F1-3s':9s} : None")
        ...
        self.ui.wMOTA_diff_label.setText(f"")
